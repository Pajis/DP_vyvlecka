...
output_table = r"D:\vysledky_modelovani_AGB\AGB_table_RF_all.csv"
model_path = r"D:\vysledky_modelovani_AGB\AGB_model_RF_all.pkl"
output_raster_path = r"D:\vysledky_modelovani_AGB\AGB_model_RF_all.tif"
feature_importance_path = r"D:\vysledky_modelovani_AGB\feature_importance_RF_all.csv"
...
if cnt_null > 0:
 ...
 # === 2. EXTRAKCE RASTROVÝCH HODNOT DO BODŮ (2x2 pixely pro les) + zápis do originálu ===
...
if not sr or sr.linearUnitName.lower() not in ["meter", "metre", "meters", "metres", "m"]:
    arcpy.AddWarning(
        ...
        "Focal 2x2 bude korektní jen v pixeleh."
    )
...
# --- param grid  ---
param_grid = {
    "n_estimators": list(range(20, 140, 10)),
    "max_depth": list(range(1, 19, 3)),
    "min_samples_split": list(range(3, 10, 1)),
    # "min_samples_leaf": [1, 2, 3, 5, 7, 10],
    "max_features": [4, 5, 6, 8, 9]
}
...
for train_idx, test_idx in outer_cv.split(X, y, groups=groups):
    ...
    model = RandomForestRegressor(random_state=36, **best_params)
    model.fit(X_tr, y_tr)
...
nested_summary_path = r"D:\vysledky_modelovani_AGB\RF2_all_nested_summary.csv"
...
best_model = RandomForestRegressor(random_state=36, **final_params)
best_model.fit(X, y)
